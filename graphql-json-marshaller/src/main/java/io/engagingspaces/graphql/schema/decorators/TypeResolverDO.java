...
public class TypeResolverDO implements TypeResolver, SchemaChildDecorator<TypeResolver, GraphQLType> {
    ...
    private TypeResolverDO(TypeResolver resolver, JsonObject resolverJson, GraphQLType parent, SchemaContext context) {
        ...
        this.id = resolverJson == null ? UUID.randomUUID().toString() : resolverJson.getString(ID);
        if (resolver == null) {
    ...
    public JsonObject toJson() {
        ...
        return new JsonObject()
                .put(MARSHALED_TYPE, TypeResolver.class.getName())
                .put(MARSHALED_TYPE_CLASS, resolver.getClass().getName())
                .put(PARENT, new JsonObject().put(REF_KEY, ((JsonSerializable) parent).jsonReference().getReference()))
                .put(ID, getId());
    }
